...
PHPC_OBJ_HANDLER_CLONE(crypto_cipher)
{
	...
	if (PHP_CRYPTO_CIPHER_AAD(PHPC_THIS)) {
		PHP_CRYPTO_CIPHER_AAD(PHPC_THAT) = emalloc(
...
PHP_CRYPTO_API const EVP_CIPHER *php_crypto_get_cipher_algorithm(zval *object TSRMLS_DC,
		char *algorithm, phpc_str_size_t algorithm_len)
{
	...
	zend_string *upper_algorithm = zend_string_toupper(zend_string_init(algorithm, algorithm_len, 0));
	cipher = EVP_get_cipherbyname(ZSTR_VAL(upper_algorithm));
	if (!cipher) {
		zend_string *lower_algorithm = zend_string_tolower(zend_string_init(algorithm, algorithm_len, 0));
		cipher = EVP_get_cipherbyname(ZSTR_VAL(lower_algorithm));
	}
	return cipher;
}
/* }}} */
...
static const EVP_CIPHER *php_crypto_get_cipher_algorithm_from_params_ex(zval *object TSRMLS_DC,
		char *algorithm, phpc_str_size_t algorithm_len,
		zval *pz_mode, zval* pz_key_size, bool is_static TSRMLS_DC)
{
	const EVP_CIPHER *cipher;
	...
	if (!pz_mode || Z_TYPE_P(pz_mode) == IS_NULL) {
		cipher = php_crypto_get_cipher_algorithm(object TSRMLS_CC, algorithm, algorithm_len);
		if (!cipher) {
			if (is_static) {
				...
				/* The continuation of the previous code from where it was cut off */
    ...
    cipher = php_crypto_get_cipher_algorithm(object TSRMLS_CC, alg_buf.c, alg_buf.len);
    if (!cipher) {
        if (is_static) {
...
PHP_CRYPTO_API const EVP_CIPHER *php_crypto_get_cipher_algorithm_from_params(zval *object TSRMLS_DC,
        char *algorithm, phpc_str_size_t algorithm_len,
        zval *pz_mode, zval* pz_key_size TSRMLS_DC)
{
    return php_crypto_get_cipher_algorithm_from_params_ex(object TSRMLS_DC, algorithm, algorithm_len, pz_mode, pz_key_size, 0 TSRMLS_CC);
}
/* }}} */

/* {{{ php_crypto_set_cipher_algorithm_ex */
static int php_crypto_set_cipher_algorithm_ex(zval *object TSRMLS_DC,
        char *algorithm, phpc_str_size_t algorithm_len TSRMLS_DC)
{
    const EVP_CIPHER *cipher = php_crypto_get_cipher_algorithm(object TSRMLS_CC, algorithm, algorithm_len);
    if (!cipher) {
        return FAILURE;
    }
...
static int php_crypto_set_cipher_algorithm(zval *object TSRMLS_DC,
        char *algorithm, phpc_str_size_t algorithm_len TSRMLS_DC)
{
    PHPC_THIS_DECLARE_AND_FETCH_FROM_ZVAL(crypto_cipher, object);
    php_crypto_cipher_set_algorithm_name(object, algorithm, algorithm_len TSRMLS_CC);
    return php_crypto_set_cipher_algorithm_ex(object TSRMLS_DC, algorithm, algorithm_len TSRMLS_CC);
}
/* }}} */

/* {{{ php_crypto_set_cipher_algorithm_from_params_ex */
static int php_crypto_set_cipher_algorithm_from_params_ex(zval *object TSRMLS_DC,
        char *algorithm, phpc_str_size_t algorithm_len,
        zval *pz_mode, zval* pz_key_size, bool is_static TSRMLS_DC)
{
    PHPC_THIS_DECLARE_AND_FETCH_FROM_ZVAL(crypto_cipher, object);
    const EVP_CIPHER *cipher = php_crypto_get_cipher_algorithm_from_params_ex(object TSRMLS_DC, algorithm, algorithm_len, pz_mode, pz_key_size, is_static TSRMLS_CC);
...
static int php_crypto_set_cipher_algorithm_from_params(zval *object TSRMLS_DC,
        char *algorithm, phpc_str_size_t algorithm_len,
        zval *pz_mode, zval* pz_key_size TSRMLS_DC)
{
    return php_crypto_set_cipher_algorithm_from_params_ex(object TSRMLS_DC, algorithm, algorithm_len, pz_mode, pz_key_size, 0 TSRMLS_CC);
}
/* }}} */
...
{
    return php_crypto_cipher_is_mode_authenticated_ex(php_crypto_get_cipher_mode_ex(PHP_CRYPTO_CIPHER_MODE_VALUE(PHPC_THIS)) TSRMLS_CC);
}
/* }}} */
...
static PHPC_OBJ_STRUCT_NAME(crypto_cipher) *php_crypto_cipher_init_ex(zval *zobject, char *key,
        phpc_str_size_t key_len, char *iv, phpc_str_size_t iv_len, int enc TSRMLS_DC)
{
    const php_crypto_cipher_mode *mode;
...
static int php_crypto_cipher_write_inlen(EVP_CIPHER_CTX *cipher_ctx, int inlen TSRMLS_DC)
{
    int outlen;
...
static int php_crypto_cipher_auth_init(PHPC_THIS_DECLARE(crypto_cipher), int inlen TSRMLS_DC)
{
...
PHP_CRYPTO_METHOD(Cipher, hasAlgorithm)
{
    ...
    if (php_crypto_get_cipher_algorithm(NULL TSRMLS_CC, algorithm, algorithm_len)) {
        RETURN_TRUE;
    } else {
        RETURN_FALSE;
    }
...
PHP_CRYPTO_METHOD(Cipher, __callStatic)
{
    ...
    if (argc == 0) {
        if (php_crypto_set_cipher_algorithm(return_value TSRMLS_CC, algorithm, algorithm_len) == FAILURE) {
            php_crypto_error_ex(PHP_CRYPTO_ERROR_ARGS(Cipher, STATIC_METHOD_NOT_FOUND), algorithm);
        }
        return;
    }
    ...
    zval *ppv_mode;
    zend_hash_get_current_data(Z_ARRVAL_P(args), (void **)&ppv_mode);
    pz_mode = ppv_mode;
    if (argc == 1) {
        pz_key_size = NULL;
    } else {
        ...
        zend_hash_get_current_data(Z_ARRVAL_P(args), (void **)&ppv_key_size);
        pz_key_size = ppv_key_size;
    }
    php_crypto_set_cipher_algorithm_from_params_ex(return_value TSRMLS_CC, algorithm, algorithm_len, pz_mode, pz_key_size, 1 TSRMLS_CC);
}
/* }}} */
...
PHP_CRYPTO_METHOD(Cipher, __construct)
{
    ...
    php_crypto_set_cipher_algorithm_from_params(getThis() TSRMLS_CC, algorithm, algorithm_len, mode, key_size);
}
/* }}} */
